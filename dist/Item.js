    ...
    save(settings, callback) {
        ...
        const paramsPromise = this.toDynamo({ "defaults": true, "validate": true, "required": true, "enum": true, "forceDefault": true, "combine": true, "saveUnknown": true, "customTypesDynamo": true, "updateTimestamps": true, "modifiers": ["set"] }).then(async (item) => {
            ...
            if (localSettings.condition) {
                putItemObj = Object.assign(Object.assign({}, putItemObj), await localSettings.condition.requestObject(this.getInternalProperties(internalProperties).model));
            }
        ...
        const promise = Promise.all([paramsPromise, this.getInternalProperties(internalProperties).model.getInternalProperties(internalProperties).table().getInternalProperties(internalProperties).pendingTaskPromise()]).then((promises) => {
...
Item.prepareForObjectFromSchema = async function (object, model, settings) {
    if (settings.updateTimestamps) {
        const schema = await model.getInternalProperties(internalProperties).schemaForObject(object);
        if (schema.getInternalProperties(internalProperties).settings.timestamps && settings.type === "toDynamo") {
            const date = Date.now();
            const createdAtProperties = (Array.isArray(schema.getInternalProperties(internalProperties).settings.timestamps.createdAt) ? schema.getInternalProperties(internalProperties).settings.timestamps.createdAt : [schema.getInternalProperties(internalProperties).settings.timestamps.createdAt]).filter((a) => Boolean(a));
            const updatedAtProperties = (Array.isArray(schema.getInternalProperties(internalProperties).settings.timestamps.updatedAt) ? schema.getInternalProperties(internalProperties).settings.timestamps.updatedAt : [schema.getInternalProperties(internalProperties).settings.timestamps.updatedAt]).filter((a) => Boolean(a));
...
Item.attributesWithSchema = async function (item, model) {
    const schema = await model.getInternalProperties(internalProperties).schemaForObject(item);
...
Item.objectFromSchema = async function (object, model, settings = { "type": "toDynamo" }) {
    ...
    const schema = settings.schema || await model.getInternalProperties(internalProperties).schemaForObject(returnObject);
