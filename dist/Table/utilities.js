...
async function getTableDetails(table, settings = {}) {
    const func = async () => {
        const tableDetails = await ddb("describeTable", { "TableName": table.getInternalProperties(internalProperties).name });
...
async function getTagDetails(table) {
    ...
    const tags = await ddb("listTagsOfResource", {
...
async function createTableRequest(table) {
    const object = Object.assign(Object.assign({ "TableName": table.getInternalProperties(internalProperties).name }, utils.dynamoose.get_provisioned_throughput(table.getInternalProperties(internalProperties).options)), await table.getInternalProperties(internalProperties).getCreateTableAttributeParams());
    ...
    return () => new Promise((resolve, reject) => {
        ...
        async function check(count) {
            ...
            if (typeof table.getInternalProperties(internalProperties).options.waitForActive !== "boolean") {
                ...
                if (count > 0) {
                    table.getInternalProperties(internalProperties).options.waitForActive.check.frequency === 0 ? await utils.set_immediate_promise() : await utils.timeout(table.getInternalProperties(internalProperties).options.waitForActive.check.frequency);
                }
                if (Date.now() - start >= table.getInternalProperties(internalProperties).options.waitForActive.check.timeout) {
                    return reject(new dynamoose_utils_1.CustomError.WaitForActiveTimeout(`Wait for active timed out after ${Date.now() - start} milliseconds.`));
                }
                else {
                    check(++count);
                }
...
async function updateTable(table) {
    ...
    if (updateAll || table.getInternalProperties(internalProperties).options.update.includes(_1.TableUpdateOptions.throughput)) {
        ...
        const expectedThroughput = utils.dynamoose.get_provisioned_throughput(table.getInternalProperties(internalProperties).options);
