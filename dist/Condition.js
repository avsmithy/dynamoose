    ...
    constructor(object) {
        ...
        if (object instanceof Condition) {
            this.setInternalProperties(internalProperties, {
                "settings": Object.assign({}, object.getInternalProperties(internalProperties).settings)
            });
        }
        else {
            this.setInternalProperties(internalProperties, {
...
Condition.prototype.requestObject = async function (model, settings = { "conditionString": "ConditionExpression", "conditionStringType": "string" }) {
    const toDynamo = async (key, value) => {
        const newValue = model ? (await Item_1.Item.objectFromSchema({ [key]: value }, model, { "type": "toDynamo", "modifiers": ["set"], "typeCheck": false }))[key] : value;
    ...
    if (this.getInternalProperties(internalProperties).settings.raw && utils.object.equals(Object.keys(this.getInternalProperties(internalProperties).settings.raw).sort(), [settings.conditionString, "ExpressionAttributeValues", "ExpressionAttributeNames"].sort())) {
        return utils.async_reduce(Object.entries(this.getInternalProperties(internalProperties).settings.raw.ExpressionAttributeValues), async (obj, entry) => {
    ...
    async function main(input) {
        return utils.async_reduce(input, async (object, entry, i, arr) => {
            let expression = "";
